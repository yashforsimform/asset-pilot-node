...
export async function findRequestsByRequester(requesterId: string): Promise<RequestWithCategoryAndItem[]> {
...
export async function findHandoversByItem(itemId: string): Promise<Prisma.HandoverRequestGetPayload<object>[]> {
...
export async function findCategoryById(id: string): Promise<Prisma.ItemCategoryGetPayload<object> | null> {
...
export async function findManagerApprovals(managerId: string): Promise<ManagerApprovalRequest[]> {
  return prisma.request.findMany({
    where: {
      managerId,
