...
import type { ApproveRequestDto, CreateRequestDto, ItemIdParams, RejectRequestDto, RequestIdParams } from "./mobile.dto";
import {
  approveRequest,
  createRequest,
...
export async function listMyDevices(req: Request, res: Response): Promise<void> {
...
export async function getMyDeviceDetail(
  ...
  const detail = await getDeviceDetail(getAuthenticatedUserId(req), req.params.itemId);
  res.json(buildSuccessResponse(detail, "Device detail fetched successfully"));
}
...
export async function listManagerApprovals(req: Request, res: Response): Promise<void> {
...
export async function approveManagerRequest(
  ...
  const updated = await approveRequest(getAuthenticatedUserId(req), req.params.requestId, req.body);
...
export async function rejectManagerRequest(
  ...
  const updated = await rejectRequest(getAuthenticatedUserId(req), req.params.requestId, req.body);
